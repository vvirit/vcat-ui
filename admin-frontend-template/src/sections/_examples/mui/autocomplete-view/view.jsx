...
export function AutocompleteView() {
  ...
  const [singleLabel, setSingleLabel] = useState(COUNTRY_LABELS[0]);
  const [multiLabel, setMultiLabel] = useState(COUNTRY_LABELS);

  const [singleCode, setSingleCode] = useState(COUNTRY_CODES[0]);
  const [multiCode, setMultiCode] = useState(COUNTRY_CODES);
...
function DisplayOutput({ value, sx, ...other }) {
  return (
    <Box
      sx={[
        {
